...
public:
	...
	void SetBlockSize(int fontw,int fonth)
	{
		block_size_x = fontw;
		block_size_y = fonth;
	}
		...
		while (appletMainLoop())
		{
			...
			if(touch_count != 0 || prev_touchcount != 0)
			{
				for(u32 i = 0 ;i<touch_count ;i++)
				{
					...
					if(touch[i].px >=0 && touch[i].px < 1280)mouse_pos_x = touch[i].px / block_size_x;
					if(touch[i].py >=0 && touch[i].py < 720)mouse_pos_y = touch[i].py / block_size_y;
					//end
